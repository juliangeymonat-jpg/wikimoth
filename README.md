...
graph, pipeline and capture are all vendored under `wikimoth/` — no `mothrag` install, no GPU, no
vector DB.
...
links — a passage *not* lexically similar to the question but *reachable by a link* still gets
...
Every stage is constructor-injectable — `MemoryRAG(retriever=…, compactor=…, reader=…)` — so you can
